...
root = '/data/p-one/mirosant/sim_10tev/DAQfiles/'
                ...
                if len(n_doms) >= j:
                    ...
                    for k in range(1, pmts+1):
                        ...
                        for l in range(len(n_doms.values())):
                            r = np.sqrt(muon.xc**2+muon.yc**2)
                            if r <= 120 and muon.zc <= 500:
...
zero = {2:{i : float() for i in rates},3:{i : float() for i in rates}}
zero_error = {2:{i : float() for i in rates},3:{i : float() for i in rates}}
...
for n in [2,3]:
    for i in fraction:
                ...
                if dom >= n:
                    for new_dom in DOMtrig[5][1][event]:
                        if new_dom >= n and new_dom < 8:
                            ...
                            eff_event = eff_event * (1-fraction[i])*dead_time
                        elif new_dom >= 8:
                            eff_event = eff_event * (1-fraction[i])
                        else:
                            continue
                    break
                else:
        ...
        zero[n][i] = (sum(efficiency)/len(efficiency))
        s = [(efficiency[k]-zero[n][i])**2 for k in range(len(efficiency))]
        zero_error[n][i] = np.sqrt(sum(s))/len(efficiency)
...
adaptive = {i: float() for i in times_ind.keys()}
adaptive_error = {i: float() for i in times_ind.keys()}
for i in times_ind:
    efficiency = []
    for event in range(len(DOMtrig[5][1])):
        eff_event = 1
        for dom in DOMtrig[5][1][event]:
            if dom > 1 and dom < 6:
                eff_event = eff_event*(1-sum(times_ind[i][:dom-1]))
            elif dom >= 6:
        ...
        efficiency.append(1-eff_event)
    adaptive[i]=(sum(efficiency)/len(efficiency))
    s = [(efficiency[n]-adaptive[i])**2 for n in range(len(efficiency))]
    adaptive_error[i] = np.sqrt(sum(s))/len(efficiency)
