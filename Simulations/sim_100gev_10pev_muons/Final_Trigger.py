...
root = '/data/p-one/mirosant/sim_100gev-10pev/DAQfiles/'
...
for i in files:
    ...
    while infile.more():
        ...
        #n_doms: number of hits in a DOM in the event.
            ...
            neutrino = frame['I3MCTree'][0].energy
            poca = frame['MMCTrackList'][0]
            if len(n_doms) > z:
                z = len(n_doms)
                    ...
                    for k in range(1, pmts+1):
                        ...
                        for l in range(len(n_doms.values())):
                            if n_doms.values()[l][0] >= k:
                                r = np.sqrt(poca.xc**2+poca.yc**2)
                                if r <= 120 and np.sqrt(poca.zc**2)<=500:
                                    flat_list = [item for sublist in n_doms.values() for item in sublist]
                                    DOMtrig[j][k].append(flat_list)
                                    neutrino_energy[j][k].append(neutrino)
                                    break
